...
describe("Error Handling", () => {
	...
	it("handles Stripe API timeout gracefully", async () => {
		vi.spyOn(stripe.checkout.sessions, "retrieve").mockRejectedValueOnce(new Error("timeout"));
...
describe("Database Constraints", () => {
	...
	it("prevents duplicate stripe session IDs", async () => {
		...
		await expect(createMember({ stripeSessionId: sessionId })).rejects.toThrow(/unique/i);
	});
});
```
...
**After initial release:** 4. Add email verification 5. Add rate limiting tests 6. Add performance tests
