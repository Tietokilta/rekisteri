...
describe("OIDC Protocol Flow Tests", () => {
  ...
  it("authorization_code WITHOUT refresh_token flow", async () => {
    ...
    const [deletedCodeRow] = await db.select().from(table.oidcEntity).where(eq(table.oidcEntity.id, code));
  ...
  it("cleanupExpiredTokens removes expired tokens and interactions", async () => {
    ...
    const [tokenRow] = await db.select().from(table.oidcEntity).where(eq(table.oidcEntity.id, "expired_token_123"));
    expect(tokenRow).toBeUndefined();

    const [interactionRow] = await db
      .select()
      .from(table.oidcEntity)
      .where(eq(table.oidcEntity.id, "expired_interaction_123"));
