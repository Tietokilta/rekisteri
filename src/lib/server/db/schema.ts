...
export const oidcEntity = snakeCase.table(
  "oidc_entity",
  {
    id: text().primaryKey(), // Interaction, refresh_token, authorization_code, Session, or Grant JTI
    kind: text(),
    uid: text(),
    grantId: text(),
  ...
  (table) => [
    index("idx_oidc_entity_uid").on(table.uid),
    index("idx_oidc_entity_grant_id").on(table.grantId),
    index("idx_oidc_entity_user_client").on(table.userId, table.clientId),
