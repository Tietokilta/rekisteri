...
export async function processPaymentReminders(
  dbInstance?: PostgresJsDatabase<Schema>,
  now?: Date,
  ...
  const membershipsWithDueDate = await db._query.membership.findMany({
  ...
  for (const membership of membershipsWithDueDate) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-deprecated
    const activeMembers = await db._query.member.findMany({
      where: and(
