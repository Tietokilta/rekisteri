...
export const load: PageServerLoad = async (event) => {
	...
	const subQuery = db
		.select({
			...
			membershipTypeName: sql<string>`${table.membershipType.name}->>'fi'`,
			membershipTypeNameEn: sql<string>`${table.membershipType.name}->>'en'`,
	...
	// Get memberships for year filters
